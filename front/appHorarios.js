 ...
 * v3 — Custom Selects animados + Toast propias sin librerías
 */
...
(function () {
    ...
    styleElement.textContent = `
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ciaf-nav-icon { font-size: .95rem; width: 1.1rem; text-align: center; flex-shrink: 0; }
        .ciaf-sidebar-footer {
        ...
        .ciaf-welcome-title { font-size: 1.6rem; font-weight: 700; letter-spacing: -.03em; }
        .ciaf-welcome-subtitle { font-size: .9rem; color: var(--muted); max-width: 360px; line-height: 1.6; }
        ...
        .ciaf-form-title { font-size: 1.2rem; font-weight: 700; letter-spacing: -.02em; margin-bottom: 1.25rem; }
        .ciaf-search-bar { display: flex; gap: .5rem; margin-bottom: 1.5rem; }
        .ciaf-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.1rem; }
        @media (max-width: 600px) { .ciaf-form-grid { grid-template-columns: 1fr; } }
        .ciaf-form-actions { grid-column: 1 / -1; display: flex; gap: .7rem; margin-top: .5rem; }
        .ciaf-field { display: flex; flex-direction: column; gap: .35rem; }
        .ciaf-label { font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: var(--muted); }
        .ciaf-input {
        ...
        .ciaf-btn-ghost { background: #f5f5f4; color: var(--muted); border: 1.5px solid var(--border); }
        .ciaf-btn-ghost:hover { background: #e7e5e4; color: var(--text); }
        .ciaf-btn-outline { background: transparent; color: var(--muted); border: 1.5px solid var(--border); }
        .ciaf-btn-outline:hover { background: var(--bg); }
        .ciaf-list-title { font-size: 1.2rem; font-weight: 700; letter-spacing: -.02em; margin-bottom: 1.25rem; }
        ...
        .ciaf-table-wrap { overflow-x: auto; border: 1.5px solid var(--border); border-radius: var(--radius); }
        .ciaf-table { width: 100%; border-collapse: collapse; font-size: .84rem; }
        .ciaf-table thead tr { background: #fafaf9; border-bottom: 2px solid var(--border); }
        .ciaf-table th {
        ...
        .ciaf-table td { padding: .7rem 1rem; border-bottom: 1px solid #f5f5f4; color: var(--text); }
        ...
        .ciaf-table-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 1rem; }
        .ciaf-count { font-size: .78rem; font-weight: 600; color: var(--muted); background: #f5f5f4; padding: .28rem .75rem; border-radius: 20px; }
        .ciaf-empty { text-align: center; color: var(--muted); padding: 2.5rem 1rem; font-size: .9rem; }
        .ciaf-exit { display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 4rem 2rem; gap: 1rem; }
        ...
        .cs-display {
            display: flex; align-items: center; justify-content: space-between;
            padding: .6rem .8rem; border: 1.5px solid var(--border); border-radius: 7px;
            background: var(--surface); cursor: pointer;
            font-family: 'Outfit', sans-serif; font-size: .9rem; color: var(--text);
            transition: border-color .15s, box-shadow .15s; min-height: 38px; width: 100%;
        }
        .cs-display:hover { border-color: #93c5fd; }
        .cs-display.cs-open { border-color: #93c5fd; box-shadow: 0 0 0 3px rgba(37,99,235,.1); }
        .cs-display.cs-disabled { background: #fafaf9; color: var(--muted); cursor: not-allowed; pointer-events: none; }
        .cs-placeholder { color: var(--muted); }
        .cs-chevron {
            display: flex; align-items: center; justify-content: center;
            width: 20px; height: 20px; border-radius: 4px;
            background: #f5f5f4; color: var(--text);
            transition: transform .25s, background .15s; flex-shrink: 0; margin-left: 8px;
        }
        .cs-display.cs-open .cs-chevron { transform: rotate(180deg); background: var(--c-listar); color: #fff; }
        .cs-dropdown {
        ...
        .cs-dropdown.cs-dropdown-open { max-height: 220px; opacity: 1; overflow-y: auto; }
        .cs-dropdown::-webkit-scrollbar { width: 4px; }
        .cs-dropdown::-webkit-scrollbar-thumb { background: var(--c-listar); border-radius: 4px; }
        .cs-option {
            padding: 9px 14px; font-family: 'Outfit', sans-serif; font-size: .875rem;
            color: var(--text); cursor: pointer;
            transition: background .15s, color .15s, padding-left .15s;
            display: flex; align-items: center; gap: 8px;
        }
        .cs-option:hover { background: #eff6ff; color: var(--c-listar); padding-left: 20px; }
        .cs-option.cs-selected { background: var(--c-listar); color: #fff; font-weight: 600; }
        .cs-option.cs-selected:hover { background: #1d4ed8; color: #fff; padding-left: 14px; }
        .cs-option-dot { width: 5px; height: 5px; border-radius: 50%; background: currentColor; flex-shrink: 0; opacity: .5; }
        .cs-empty { padding: 12px 14px; font-size: .82rem; color: var(--muted); font-style: italic; text-align: center; }
    `;
    ...
    function crearCustomSelect({ id, placeholder, opciones, deshabilitado, onChange }) {
        ...
        function poblarOpciones(lista) {
            ...
            valorActual = ""; hiddenInput.value = "";
            txtSpan.textContent = placeholder; txtSpan.className = "cs-placeholder";
            ...
            lista.forEach(function (opcion) {
                ...
                opt.className = "cs-option"; opt.setAttribute("role", "option"); opt.dataset.value = opcion;
                const dot = document.createElement("span"); dot.className = "cs-option-dot";
                const lbl = document.createElement("span"); lbl.textContent = opcion;
                opt.appendChild(dot); opt.appendChild(lbl);
                opt.addEventListener("click", function (e) { e.stopPropagation(); seleccionarValor(opcion); cerrar(); });
                dropdown.appendChild(opt);
            });
        }
        function seleccionarValor(val) {
            valorActual = val; hiddenInput.value = val;
            txtSpan.textContent = val; txtSpan.className = "";
        ...
        function resetValor(nuevoPlaceholder) {
            valorActual = ""; hiddenInput.value = "";
            txtSpan.textContent = nuevoPlaceholder || placeholder; txtSpan.className = "cs-placeholder";
            dropdown.querySelectorAll(".cs-option").forEach(function (o) { o.classList.remove("cs-selected"); });
        }
        function abrir() {
            if (deshabilitado) return; abierto = true;
            display.classList.add("cs-open"); display.setAttribute("aria-expanded", "true");
            dropdown.classList.add("cs-dropdown-open");
        }
        function cerrar() {
            abierto = false; display.classList.remove("cs-open");
            display.setAttribute("aria-expanded", "false"); dropdown.classList.remove("cs-dropdown-open");
        }
    ...
    // ── Feedback ───────────────────────────────────────────────────
    function lanzarMensajeFeedback(zona, texto, esError) {
    ...
    function componenteFormulario(modo) {
        ...
        if (modo === "editar" || modo === "borrar") {
            ...
            btnBuscar.addEventListener("click", async () => {
                ...
                try {
                    ...
                    mostrarToast("Horario consultado correctamente.", "listar");
        ...
        csF = crearCustomSelect({
            id: "formFacultad", placeholder: "-- Seleccione facultad --",
            opciones: Object.keys(DATOS_CIAF), deshabilitado: modo === "borrar",
        ...
        csC = crearCustomSelect({
            id: "formCarrera", placeholder: "-- Seleccione carrera --",
            opciones: [], deshabilitado: modo === "borrar",
            onChange: function (val) {
        ...
        csM = crearCustomSelect({ id: "formMateria", placeholder: "-- Seleccione materia --", opciones: [], deshabilitado: modo === "borrar" });
        ...
        [
            ...
            divCol.appendChild(crearNodo("label", { class: "ciaf-label", text: campo.label, for: campo.id }));
            const inputCampo = crearNodo("input", { type: campo.type, class: "ciaf-input", id: campo.id });
        ...
        btnSubmit.addEventListener("click", async () => {
            ...
            if (modo === "borrar") {
                ...
                try {
                    ...
                    mostrarToast("¡Horario eliminado correctamente!", "borrar");
            ...
            if (modo === "editar") {
                if (!window.confirm("¿Está seguro de editar este horario?")) return;
            }
            ...
            try {
                if (modo === "crear") {
                    ...
                    btnSubmit.setAttribute("disabled", "true");
                    mostrarToast("¡Horario creado exitosamente!", "crear");
                    setTimeout(() => transicionarModulo("inicio"), 1500);
                } else if (modo === "editar") {
                    ...
                    btnSubmit.setAttribute("disabled", "true");
                    mostrarToast("¡Horario editado correctamente!", "editar");
                    setTimeout(() => transicionarModulo("inicio"), 1500);
                }
            } catch (ex) {
    ...
    function componenteTablaListado() {
        ...
        const inputBusq = crearNodo("input", { type: "text", class: "ciaf-input-sm", placeholder: "Buscar...", value: moduloEstado.filtrosTable.buscarTermino });
        ...
        async function cargarDatos() {
            ...
            try {
                ...
                if (terminoBusqueda) {
                    arr = arr.filter(item =>
                        Object.values(item).some(valor => String(valor ?? "").toLowerCase().includes(terminoBusqueda))
                    );
                }
                const campoOrden = moduloEstado.filtrosTable.ordenador;
                arr = arr.sort((a, b) => {
                    if (campoOrden === "idHorario") return (parseInt(a.idHorario) || 0) - (parseInt(b.idHorario) || 0);
